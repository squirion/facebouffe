...
function RecipeScreen({ route }) {
  ...
  const recipe = app.getRecipe(route.id);
  ...
  const members = group ? group.memberIds.map(app.getRecipe).filter(Boolean) : [];
  const linked = (recipe.links || []).map(app.getRecipe).filter(Boolean);
    ...
    <div style={{ height: "100%", display: "flex", flexDirection: "column", position: "relative" }}>
      <div className="fb-scroll" style={{ flex: 1, overflowY: "auto", overflowX: "hidden", paddingBottom: 32 + app.insets.bottom }}>
        ...
        <div style={{ position: "relative" }}>
          ...
          <div style={{ position: "absolute", top: app.insets.top + 6, left: 14, right: 14, display: "flex", justifyContent: "space-between", pointerEvents: "none" }}>
            <RoundBtn icon="back" onClick={() => nav.back()} />
            <div style={{ display: "flex", gap: 8, pointerEvents: "auto" }}>
              <RoundBtn icon="star" fill={fav} color={fav ? th.gold : "#fff"} pulse={favPulse} reduce={app.reduceMotion} onClick={() => { app.toggleFav(recipe.id); setFavPulse((p) => p + 1); }} />
              <RoundBtn icon="pencil" onClick={() => nav.edit(recipe.id)} />
            </div>
          </div>
        </div>

        <div style={{ padding: "0 20px", marginTop: -26, position: "relative", zIndex: 2 }}>
          ...
          {/* Steps */}
          <h2 style={{ margin: "30px 0 14px", fontFamily: th.fontDisplay, fontSize: th.fs(22), fontWeight: 600, color: th.ink }}>{t("steps")}</h2>
    ...
    <div style={{ height: "100%", position: "relative", overflow: "hidden", color: sc.text }}>
      ...
      <div style={{ position: "relative", zIndex: 1, height: "100%", display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ flex: 1, overflow: "hidden", position: "relative" }} onTouchStart={onTouchStart} onTouchMove={onTouchMove} onTouchEnd={onTouchEnd}>
          <div style={{ display: "flex", width: "100%", height: "100%", transform: `translateX(calc(${pane * -100}% + ${drag}px))`, transition: dragging ? "none" : (app.reduceMotion ? "transform 120ms ease" : "transform 0.34s cubic-bezier(0.4,0,0.2,1)") }}>
            {/* Pane 1: ingredients checklist */}
            <div style={{ flex: "0 0 100%", width: "100%", height: "100%", overflowY: "auto", padding: `8px 22px ${30 + app.insets.bottom}px` }} className="fb-scroll">
              {recipe.ingredients.map((ing, i) => {
                const d = FB.displayIngredient(ing, ratio, prefs, lang);
                const on = checked[i];
                return (
            ...
            {/* Pane 2: step-by-step */}
            <div style={{ flex: "0 0 100%", width: "100%", height: "100%", display: "flex", flexDirection: "column" }}>
              {finished ? (
